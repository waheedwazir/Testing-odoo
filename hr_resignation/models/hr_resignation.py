...
date_format = "%Y-%m-%d"
RESIGNATION_TYPE = [('resigned', 'Resignation'),
                    ('end_con', 'End of contract'),
                    ('inv_sep', 'Involuntary Separation')]
...
class HrResignation(models.Model):
    ...
    name = fields.Char(string='New Resignation Request', required=True, copy=False, readonly=True, index=True,
                       default=lambda self: _('New Resignation Request'))
    ...
    resign_confirm_date = fields.Date(string="Confirmed Date 111",
                                      help='Date on which the request is confirmed by the employee.',
                                      track_visibility="always")
    approved_revealing_date = fields.Date(string="Last Working Day Approved by the LM",
                                          help='Date on which the request is confirmed by the manager.',
                                          track_visibility="always")
    joined_date = fields.Date(string="Joining Date", store=True,
                              help='Joining date of the employee.i.e Start date of the first contract')

    expected_revealing_date = fields.Date(string="Last Working Day ", required=True,
    ...
    notice_period = fields.Selection([('fully_served', 'Fully Served'), ('not_served', 'Not Served'), ('partially_served', 'Partially Served'), ('not_applicable', 'Not Applicable'),
                                     ('waved','Waved')],string='Notice Period')
    ...
    # count = fields.Integer(compute='request_hiring')

    # def request_hiring(self):
    #     self.ensure_one()
    #     return {
    #         'type': 'ir.actions.act_window',
    #         'name': 'Hiring Request',
    #         'view_mode': 'tree,form',
    #         'res_model': 'hiring.recuritment',
    #         # 'domain': [('id', '=', self.id)],
    ...
    # def request_hiring(self):
    #     for record in self:
    #         record.count = self.env['hiring.recuritment'].search_count(
    #             [])
...
class HrEmployee(models.Model):
    ...
    count = fields.Integer(compute='count_resignation')

    def show_resignation(self):
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': 'Resignation',
    ...
    def count_resignation(self):
        for record in self:
            record.count = self.env['hr.resignation'].search_count([('employee_id', '=', self.id)])